...
public class LinkedBinaryTree<T> implements BinaryTreeADT<T>
{
	...
	public Iterator<T> pathToRoot(T targetElement) throws ElementNotFoundException{
		...
		//We call pathToRootAgain to build the ArrayUnorderedList we created so it is a path to the root
		ArrayUnorderedList<T> pathToRoot = new ArrayUnorderedList<T>();
		pathToRootAgain( targetElement, root, pathToRoot );
		//We check to see if the element is even found
	...
	protected void pathToRootAgain(T targetElement, BinaryTreeNode<T> node, ArrayUnorderedList<T> pathToRoot){
		// complete for Assignment 4
		//We recursively move down the tree until we find the target element.
		//We then follow our path back up the tree adding on elements to the 
		//List, which creates the path.
		//We don't add any other nodes, because we only start adding elements once we found the path.
		if (node != null)
		{
	...
	public T lowestCommonAncestor( T targetOne, T targetTwo) throws ElementNotFoundException{
		...
		//Find the path to the root for two elements in the tree and get iterators for them.
		Iterator<T> one = this.pathToRoot(targetOne);	
		Iterator<T> two = this.pathToRoot(targetTwo);
		ArrayUnorderedList<T> onPathOne = new ArrayUnorderedList<T>();

		//Add all the elements in on tree to a list
		while (one.hasNext())
		{
			onPathOne.addToRear(one.next());
		}
		T temp;
		//In order check to see if there are any nodes matching in the tree
		//We iterate through the second list, and check the arrayunorderedlist previously created
		while(two.hasNext())
		{
			temp = two.next();
			if (onPathOne.contains(temp))
				//We return a common ancestor
				return temp;
		}
		return root.element;
	}
