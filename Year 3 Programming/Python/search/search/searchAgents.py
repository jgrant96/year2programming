...
class CornersProblem(search.SearchProblem):
  ...
  def getSuccessors(self, state):
	...
	nextPosition = (nextState, cornersLeft)
        cost = 1
...
def cornersHeuristic(state, problem):
  ...
  if len(cornersToFind) == 0:
	return 0
  while len(cornersToFind) > 0:
  	distance = euclideanDistance(currentPoint, cornersToFind[0])
  	closestCorner = cornersToFind[0]
  	for corners in cornersToFind:
		if (distance > euclideanDistance(currentPoint, corners)):
			distance = euclideanDistance(currentPoint, corners)
			closestCorner = corners
	heuristic = heuristic + euclideanDistance(currentPoint, closestCorner)
	cornersToFind.remove(closestCorner)
